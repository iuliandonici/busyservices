#!/bin/bash
# var_f_config_kvm_images_arch_alpine=("x86" "x86_x64" "aarch64")
...
# for i in "${!var_f_config_kvm_images_arch_alpine[@]}"
# do
#     wget -r --no-parent -X "*/*/*/*/netboot*/" -A "alpine-standard-*.*.*-x86_64.iso" -R "alpine-standard-*.*.*_rc*-x86_64.iso" -N https://dl-cdn.alpinelinux.org/alpine/latest-stable/releases/$var_f_config_kvm_images_arch_alpine[$i]/
#     echo " $i ${var_f_config_kvm_images_arch_alpine[$i]}"
# done
