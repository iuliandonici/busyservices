#!/bin/bash
var_install_docker_software_array=("docker-ce" "docker-ce-cli" "containerd.io" "docker-compose-plugin")
function f_install_docker() {
    ...
    do
        echo " $i ${var_install_docker_software_array[$i]}"
    done
    if [[ $(f_get_distro_packager) == "apk" ]]; then
        for i in "${!var_install_docker_software_array[@]}"
        do
            echo "- Currently installing: $i ${var_install_docker_software_array[$i]}"
            if [[ "$EUID" -ne 0 ]]; then 
                sudo $(f_get_distro_packager) add ${var_install_docker_software_array[$i]}  
            else
                $(f_get_distro_packager) add ${var_install_docker_software_array[$i]}  
            fi
        done     
