...
function f_config_transmission() {
    ...
    if [[ -f /usr/bin/transmission-daemon ]]; then
        ...
        if [[ "$EUID" -ne 0 ]]; then 
            ...
            sudo sed -i -e 's/Type=notify/Type=simple/g' /etc/systemd/systemd/transmission-daemon.service
            ...
            sed -i -e 's/Type=notify/Type=simple/g' /etc/systemd/systemd/transmission-daemon.service
