...
function f_install_brave_browser() {
    ...
    if [[ $(f_check_networks) == "UP" ]]; then
        ...
        if [[ $(f_get_distro_packager) == "apk" ]]; then
            ...
            if [[ "$EUID" -ne 0 ]]; then        
                $(f_get_security_utility) $(f_get_distro_packager) install -y dnf-plugins-core
                $(f_get_security_utility) $(f_get_distro_packager) config-manager addrepo --from-repofile=https://brave-browser-rpm-release.s3.brave.com/brave-browser.repo
                $(f_get_security_utility) curl -fsSLO "https://dl.brave.com/install.sh{,.asc}" && gpg --keyserver hkps://keys.openpgp.org --recv-keys D16166072CACDF2C9429CBF11BF41E37D039F691 && gpg --verify install.sh.asc
                ...
                $(f_get_security_utility) rpm --import https://brave-browser-rpm-release.s3.brave.com/brave-core.asc
            else
            ...
            if [[ "$EUID" -ne 0 ]]; then 
                $(f_get_security_utility) $(f_get_distro_packager) addrepo https://brave-browser-rpm-release.s3.brave.com/brave-browser.repo
                $(f_get_security_utility) $(f_get_distro_packager) rpm --import https://brave-browser-rpm-release.s3.brave.com/brave-core.asc
            else
            ...
            if [[ "$EUID" -ne 0 ]]; then 
                $(f_get_security_utility) curl -fsSLo /usr/share/keyrings/brave-browser-archive-keyring.gpg https://brave-browser-apt-release.s3.brave.com/brave-browser-archive-keyring.gpg
