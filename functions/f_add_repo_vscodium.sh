...
function f_add_repo_vscodium() {
    ...
    if [[ "$(f_get_distro_packager)" == "dnf" ]]; then
        if [[ "$EUID" -ne 0 ]]; then # Setting a variable for getting the machine's architecture
            ...
            $(f_get_security_utility) rpmkeys --import https://gitlab.com/paulcarroty/vscodium-deb-rpm-repo/-/raw/master/pub.gpg
            architecture=$(uname -m)
            if [[ $architecture == "x64" || $architecture == "x86_64" ]]; then
                printf "[gitlab.com_paulcarroty_vscodium_repo]\nname=download.vscodium.com\nbaseurl=https://download.vscodium.com/rpms/\nenabled=1\ngpgcheck=1\nrepo_gpgcheck=1\ngpgkey=https://gitlab.com/paulcarroty/vscodium-deb-rpm-repo/-/raw/master/pub.gpg\nmetadata_expire=1h\n" | $(f_get_security_utility) tee -a /etc/yum.repos.d/vscodium.repo
            else
...
# Setting a variable for getting the machine's architecture
        ...
        if [[ "$EUID" -ne 0 ]]; then # Setting a variable for getting the machine's architecture
            echo "- Adding the GPG key:"
            $(f_get_security_utility) $(f_get_distro_packager) --gpg-auto-import-keys https://gitlab.com/paulcarroty/vscodium-deb-rpm-repo/-/raw/master/pub.gpg
            architecture=$(uname -m)
            if [[ $architecture == "x64" || $architecture == "x86_64" ]]; then
                printf "[gitlab.com_paulcarroty_vscodium_repo]\nname=gitlab.com_paulcarroty_vscodium_repo\nbaseurl=https://download.vscodium.com/rpms/\nenabled=1\ngpgcheck=1\nrepo_gpgcheck=1\ngpgkey=https://gitlab.com/paulcarroty/vscodium-deb-rpm-repo/-/raw/master/pub.gpg\nmetadata_expire=1h\n" | $(f_get_security_utility) tee -a /etc/zypp/repos.d/vscodium.repo
            else
                echo "There is no version of VSCodium for x86."
            fi            
        else
...
# Setting a variable for getting the machine's architecture
        ...
        if [[ "$EUID" -ne 0 ]]; then 
            $(f_get_security_utility) wget https://gitlab.com/paulcarroty/vscodium-deb-rpm-repo/raw/master/pub.gpg \
...
# Setting a variable for getting the machine's architecture
            ...
            if [[ $architecture == "x64" || $architecture == "x86_64" ]]; then
                ...
                | $(f_get_security_utility) tee /etc/apt/sources.list.d/vscodium.list
            else
                echo "There is no version of VSCodium for x86."
            fi            
        else
