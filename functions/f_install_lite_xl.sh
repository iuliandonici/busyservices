  ...
  if [[ $(f_get_distro_packager) == "apk" ]]; then
    if [[ "$EUID" -ne 0 ]]; then
      if grep -wq "edge/testing" /etc/apk/repositories; then
        ...
        $(f_get_security_utility) chown -R $USER:$USER /etc/apk/repositories
        echo "- currently adding the the edge testing Alpine repository;"
        $(f_get_security_utility) echo "http://dl-cdn.alpinelinux.org/alpine/edge/testing" >> /etc/apk/repositories
      fi
      echo "- currently installing lite-xl;" 
      if [[ $(f_check_networks) == "UP" ]]; then
        f_update_software
        $(f_get_security_utility) $(f_get_distro_packager) add lite-xl
        ...
        $(f_get_security_utility) mv lpm /usr/bin/
