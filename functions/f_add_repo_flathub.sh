...
function f_add_repo_flathub() {
  ...
  if [[ $(f_get_distro_packager) == "apk" ]]; then
    ...
    if [[ "$EUID" -ne 0 ]]; then
      # Install the package
      $(f_get_security_utility) apk add flatpak
      ...
      if [[ $var_f_add_repo_flathub_desktop == "KDE" ]]; then
        $(f_get_security_utility) apk add discover-backend-flatpak
      # or Gnome
      elif [[ $var_f_add_repo_flathub_desktop == "Gnome" ]]; then
        $(f_get_security_utility) apk add gnome-software-plugin-flatpak
      fi
