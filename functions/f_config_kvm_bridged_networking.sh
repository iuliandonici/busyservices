...
function f_config_kvm_bridged_networking() {
    ...
    if [[ $(f_get_distro_packager) == "apk" ]]; then
        if [[ "$EUID" -ne 0 ]]; then 
            doas cp -r functions/f_config_kvm_netfilter_bridge /etc/sysctl.d/99-kvm-netfilter-bridge.conf
            doas ip link add bridge0 type bridge
            # sudo ip link set ens33 master bridge0
            doas ip address add dev bridge0 192.168.50.0/24
            doas modprobe br_netfilter
            doas echo "br_netfilter" >> br_netfilter.conf
            doas mv br_netfilter.conf /etc/modules-load.d/br_netfilter.conf
            doas sysctl -p /etc/sysctl.d/99-kvm-netfilter-bridge.conf
        else
